			...
			Case $GUI_UserSettings
				Switch $aMsg[0] ; Now check for the messages for $GUI_UserSettings
					...
					Case $BTN_Save
						...
						Else
						$settingsConfirm = IniWrite(@ScriptDir & '\settings.ini', 'Settings', 'DropboxDir', $DATA_UserSettings)
						If $settingsConfirm = 1 Then
...
Func OpenSettingsGUI()
	$GUI_UserSettings = GUICreate("User Settings", 236, 96, -1, -1)
	 $INPT_DropboxFolder = GUICtrlCreateInput("", 16, 16, 201, 21)
	 $BTN_Save = GUICtrlCreateButton("Save", 16, 48, 75, 25)
	 $BTN_Cancel = GUICtrlCreateButton("Cancel", 144, 48, 75, 25)
	GUISetState()
